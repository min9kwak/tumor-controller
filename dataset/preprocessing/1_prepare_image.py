...
for patient_dir in tqdm.tqdm(patient_dirs, desc="Patients"):
    ...
    t1_path = next(f for f in os.listdir(patient_dir) if '_t1.nii' in f and '_t1ce' not in f)
    ...
    t1_raw = nib.load(os.path.join(patient_dir, t1_path)).get_fdata().transpose(2,0,1)
    ...
    t1_padded = padding_function(t1_raw[np.newaxis]).numpy()[0]      # (155, 256, 256)
    ...
    t1ce_01, t1ce_min, t1ce_max = normalize_volume_01(t1ce_padded)
    t1_01, t1_min, t1_max = normalize_volume_01(t1_padded)
    t2_01, t2_min, t2_max = normalize_volume_01(t2_padded)

    stat_records += [
        {"patient_id": patient_id, "modality": "t1ce", "min": t1ce_min, "max": t1ce_max},
        {"patient_id": patient_id, "modality": "t1", "min": t1_min, "max": t1_max},
    ...
    for z in slice_range:
        ...
        with open(f"{prefix}-brain-t1-{z}.pkl", "wb") as f:
            pickle.dump(t1_slice, f)
        ...
        with open(f"{prefix}-edge-t1-{z}.pkl", "wb") as f:
            pickle.dump(canny_t1, f)
        ...
        with open(f"{prefix}-edge_blur-t1-{z}.pkl", "wb") as f:
            pickle.dump(blur_canny_t1, f)
